...
for result_filename in p['result_filenames']:
  ...
  for i, scene_id in enumerate(targets_org):
    ...
    if dp_split['name'] != 'itodd':
      if dp_split['split_type'] is None:
        path = './results/' + dataset + '_' + method + '/' + dp_split['split'] + '/' + "{:06d}".format(scene_id) + '/' + dp_split['im_modalities'][0]
      else:
        path = './results/' + dataset + '_' + method + '/' + dp_split['split'] + '_' + dp_split['split_type'] + '/' + "{:06d}".format(scene_id) + '/' + dp_split['im_modalities'][0]
      ...
      for img in scene_coco_ann['images']:
        if img['id'] in target_img_ids:
          print(dp_split['rgb_tpath'].format(scene_id=scene_id, im_id=img['id']) + ' -> ' + path + '/' + "{:06d}".format(img['id']) + '.jpg')
          
          image = cv2.imread(dp_split['rgb_tpath'].format(scene_id=scene_id, im_id=img['id'])) 
          ...
          for ann in scene_coco_ann['annotations']:
            if ann['image_id'] in target_img_ids and ann['image_id'] == img['id']:
              [x,y,w,h] = ann['bbox']
              cv2.rectangle(image_rgb, (int(x), int(y)), (int(x+w), int(y+h)), (0,0,255), 3)
          plt.imshow(image_rgb)
          plt.axis('off')  # Turn off axis labels
          plt.show()

          for ann in scene_coco_results:
            if ann['image_id'] in target_img_ids and ann['image_id'] == img['id']:
              [x,y,w,h] = ann['bbox']
              cv2.rectangle(image_rgb, (int(x), int(y)), (int(x+w), int(y+h)), (0,255,0), 3)
          plt.imshow(image_rgb)
          plt.axis('off')  # Turn off axis labels
          plt.show()

          plt.imsave(path + '/' + "{:06d}".format(img['id']) + '.jpg', image_rgb)
      ...
      scene_coco_results = results_org[scene_id] if scene_id in results_org else []
      scene_coco_results = [res for res in scene_coco_results if res["image_id"] in target_img_ids]
      for img in target_img_ids:
        print(dp_split['rgb_tpath'].format(scene_id=scene_id, im_id=img) + ' -> ' + path + '/' + "{:06d}".format(img) + '.jpg')
        image = cv2.imread(dp_split['gray_tpath'].format(scene_id=scene_id, im_id=img))
        image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        plt.imshow(image_rgb)
        plt.axis('off')  # Turn off axis labels
        # plt.show()
        ...
        for ann in scene_coco_results:
          if ann['image_id'] in target_img_ids and ann['image_id'] == img and ann['score'] > 0.5:
            [x,y,w,h] = ann['bbox']
            cv2.rectangle(image_rgb, (int(x), int(y)), (int(x+w), int(y+h)), (0,255,0), 3)
        plt.imshow(image_rgb)
        plt.axis('off')  # Turn off axis labels
        ...
        plt.imsave(path + '/' + "{:06d}".format(img) + '.jpg', image_rgb)
